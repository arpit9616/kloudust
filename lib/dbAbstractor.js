 ...
 */
exports.addHostToDB = async (hostname, rootid, rootpw, hostkey) => {
    ...
    const query = "insert into hosts(hostname, rootid, rootpw, hostkey) values (?,?,?,?)";
    return db.runCmd(query, [hostname, rootid, rootpw_encrypted, hostkey]);
}

/**
 * Retuns the host entry object for the given hostname
...
exports.getHostEntry = async hostname => {
    ...
    const hosts = db.runCmd("select * from hosts where hostname = ?", hostname);
