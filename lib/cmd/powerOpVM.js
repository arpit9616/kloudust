 ...
 * @param {array} params The incoming params - must be - type (centos8 only for now), ip, user id, password, ssh hostkey, VM name, [start|stop|forcestop|reboot|autostart|noautostart] - default is start
 */
    ...
    const xforgeArgs = {
        ...
        other: [
            ...
            params[5], params[6].toLowerCase()=="start"?"start":(params[6].toLowerCase()=="stop"?"shutdown":
                (params[6].toLowerCase()=="reboot"?"reboot":(params[6].toLowerCase()=="forcestop"?"destroy":
                (params[6].toLowerCase()=="autostart"?"autostart": (params[6].toLowerCase()=="noautostart"?
                "autostart --disable":"start")))))
        ]
    }
