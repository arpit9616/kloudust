...
const DB_PATH = `${KLOUD_CONSTANTS.ROOTDIR}/db/kloudust.db`;
const DB_CREATION_SQLS = [
    ...
    "CREATE TABLE hosts(hostname varchar not null primary key, rootid varchar, rootpw varchar)",
    "COMMIT TRANSACTION"
]
...
async function _createDB() {
    try {await accessAsync(DB_PATH, fs.constants.F_OK | fs.constants.W_OK); return true;} catch (err) {  // db doesn't exist
